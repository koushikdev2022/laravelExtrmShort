                    ...
                    <div class="main_dashbord_area">
                        <div class="earings_box">
                            <div class="earning_txt">
                                <h4 class="text_uppercase mb-0">My Bookmark Videos</h4>
                            </div>
                                    ...
                                    @forelse($data as $p)
                                        ...
                                        <div class="col-lg-4 col-md-6 dsh_bx">
                                            <div class="dash_grid_box">
                                                <a href="javascript:void(0)" onclick="showVideo({{ $p->project->id }})">
                                                    <div class="grid_video">
                                                        <video poster="{{ URL::asset('public/uploads/frontend/project/img_preview/'.$p->project->image) }}" muted>
                                                            <source src="{{ URL::asset('public/uploads/frontend/project/video/'.$p->project->video) }}" type="video/mp4">
                                                        </video>
                                                        @if($p->project->is_exclusive == '1')
                                                ...
                                                $category = \App\Models\TranslationCategory::where(['category_id'=>$p->project->category])->where('lang_code', '=', $lang)->first();
                                                @endphp

                                                <a href="javascript:void(0)">
                                                    <h5>{{ \Illuminate\Support\Str::limit(ucfirst($p->project->title),150) }}</h5>
                                                </a>
                                                @if($p->project->category != '')
                                                <a href="javascript:void(0)" class="grid_btn">{{ ucfirst($category->category_name) }}</a>
                                                @endif
                                                <ul>
                                                    @if(count($p->project->info) > 0)
                                                    @forelse($p->project->info as $key =>$i)
                                                        @if($key < 1 )
                                                ...
                                                <div class="row amountbx">
                                                    <div class="col-md-6 amnt_bx">
                                                        <h4>{{ date("F d, Y", strtotime($p->project->created_at)) }}</h4>
                                                        <h5>{{ date("h:i A", strtotime($p->project->created_at)) }}</h5>
                                                    </div>
                                                    <div class="col-md-6 amnt_bx">
                                                        <h4>amount received</h4>
                                                        @php
                                                        $total = '0';
                                                        $pays = \App\Models\Payments::where(['user_id'=>$p->project->user->id,'status'=>'approved'])->get();
                                                        if(count($pays) > 0){
                                                        foreach($pays as $p){
                                                        $total += $p->amount;
                                                        }
                                                ...
                                                <div class="thumnel_edit">
                                                    <a href="{{ Route('editUpload',base64_encode($p->project->id))}}" class="grid_btn"><i class="icofont-pencil-alt-2"></i>Edit</a>
                                                    <a href="javascript:void(0)" class="grid_btn" onclick="deleteVideo({{ $p->project->id }})"><i class="icofont-trash"></i>Delete</a>
                                                </div>
                                            </div>
                                        </div>
                                        @endif
                                    ...
                                    <table>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td onclick="showVideo({{ $d->id }})">@if($d->category != '')<a href="javascript:void(0)" class="btn_tble">{{ ucfirst($category->category_name) }}</a>@endif</td>
