...
class CareerController extends AdminController {
    ...
    public function view(Request $request, $id) {
        $data = [];
        $data['model'] = $model = Career::findOrFail($id);
        if ($model) {
    ...
    public function report1() {
        ...
        if(isset($_GET['search_filter'])){
            $title = $_GET['title'];
            $data = DB::table('reports')->join('project', 'reports.project_id','=', 'project.id')->where('project.title','like', $title.'%')
            ->select('reports.*', 'project.title as projectTitle', 'project.status as projectStatus')->get();

        }else{

            $data = DB::table('reports')->join('project', 'reports.project_id','=', 'project.id')
            ->select('reports.*', 'project.title as projectTitle', 'project.status as projectStatus')->get();
        }

        return view('admin::contact.reports', ['data'=>$data]);
        // echo 'error';
    ...
    public function send_reply(Request $request) {
        ...
        return redirect()->route('admin-viewcareer', ['id' => $model->id])->withErrors($validator)->withInput();
    }
