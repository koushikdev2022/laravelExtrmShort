            ...
            <form class="form-horizontal form-row-seperated" action="{{ Route('admin-updateuser', ['id' => $model->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-body">
                    <div class="form-group {{ $errors->has('profile_picture') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-8">
                            <img src="{{($model->profile_picture != '') ? asset('storage/uploads/frontend/profile_picture/preview/'.$model->profile_picture) : asset('storage/frontend/images/profile_user.png')}}" alt="" style="width:150px;height:150px;" class="form-control">
                            <input type="file" name="profile_picture" placeholder="Choose Profile Picture" value="{{($model->profile_picture != '') ? $model->profile_picture : ''}}">
                    ...
                    <div class="form-group {{ $errors->has('username') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-8">
                            <input type="text" class="form-control" name="username" value="{{ (old('username') != "") ? old('username') : $model->username }}" placeholder="Username">
                    ...
                    <div class="form-group {{ $errors->has('country') ? ' has-error' : '' }}">
                        ...
                        <div class="col-md-8">
                            <select name="country" class="form-control">
                                    ...
                                    <option value="{{$country->id}}" {{($model->country == $country->id) ? 'selected' : ''}}>{{$country->country_name}}</option>
                                    @endforeach
                            </select>
                        ...
                        <div class="col-md-8">
                            <div class="radio-list">
                                <label class="radio-inline">
                                    <input type="radio" name="status" value="1" {{ (old('status') != '') ? ((old('status') == 1) ? 'checked' : "") : (($model->status == 1) ? 'checked' : '') }}> Active
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="status" value="0" {{ (old('status') != '') ? ((old('status') == 0) ? 'checked' : "") : (($model->status == 0) ? 'checked' : '') }}> Inactive
                                </label>
